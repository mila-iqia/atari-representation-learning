...
class ProbeTrainer(Trainer):
    def __init__(self, encoder, wandb, info_dict,
        ...
        self.early_stoppers = {k: EarlyStopping(patience=patience, verbose=False, wandb=self.wandb, name=k + "_probe") for k in
                               info_dict.keys()}
    ...
    def do_one_epoch(self, episodes, label_dicts):
        epoch_loss, accuracy = {k + "_loss": [] for k in self.info_dict.keys() if not self.early_stoppers[k].early_stop}, \
    ...
    def train(self, episodes, label_dicts):
        ...
        for e in range(self.epochs):
            ...
            val_epoch_loss, val_accuracy = self.test(val_eps, val_labels,epoch=e, prefix="val_")
            
            #update all early stoppers
            for k in self.info_dict.keys():
                if not self.early_stoppers[k].early_stop:
                    self.early_stoppers[k](val_epoch_loss["val_" + k + "_loss"],self.probes[k])
    ...
    def test(self, test_episodes, test_label_dicts, epoch=None, prefix="test_"): 
        if prefix == "test_":
