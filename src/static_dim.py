...
class StaticDIMTrainer(Trainer):
    def __init__(self, encoder, config, device=torch.device('cpu'), wandb=None):
        ...
        self.device = device
        self.num_classifiers = 88
        self.classifiers = [nn.Linear(self.encoder.hidden_size, 128).to(device) for _ in range(self.num_classifiers)]
        self.params = list(self.encoder.parameters())
        for clf in self.classifiers:
            self.params += list(clf.parameters())
    ...
    def do_one_epoch(self, epoch, episodes):
        ...
        for x_t in data_generator:
            f_t_maps = self.encoder(x_t, fmaps=True) #, self.encoder(x_tprev, fmaps=True)

            # Loss 1: Global at time t, f5 patches at time t
            ...
            N = f_t.size(0)
            loss = 0.
            for clf_in in range(self.num_classifiers):
