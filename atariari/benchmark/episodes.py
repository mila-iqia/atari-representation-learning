...
def get_episodes(env_name,
    ...
    episodes, episode_labels = get_rollouts(env_name=env_name,
                                           steps=steps,
                                           seed=seed,
    ...
    elif train_mode == "dry_run":
        return episodes, episode_labels
    else:
        assert False, "invalid train mode"

def get_rollouts(env_name, steps, seed=42, num_processes=1,
                     num_frame_stack=1, downsample=False, color=False, collect_mode="random_agent", checkpoint_index=-1):

    if collect_mode == "pretrained_ppo":
